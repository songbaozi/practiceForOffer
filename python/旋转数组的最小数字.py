# -*- coding:utf-8 -*-





#O(n)解法，遍历数组
class Solution:
    def minNumberInRotateArray(self, rotateArray):
        # write code here
        if len(rotateArray) == 0:
            return 0
        for i in range(len(rotateArray) - 1):
            if rotateArray[i] > rotateArray[i+1]:
                return rotateArray[i+1]
        return rotateArray[-1]


#
class Solution2:
    def minNumberInRotateArray(self, rotateArray):
        # write code here
        left = 0
        right = len(rotateArray) - 1
        minIndex = left
        while rotateArray[left] >= rotateArray[right]:
            if right - left == 1:
                minIndex = right
                break
            minIndex = (right + left) / 2
            # 左指针指向的数，右指针指向的数以及最小指针指向的数都相等时
            if rotateArray[left] == rotateArray[right] and \
                            rotateArray[left] == rotateArray[minIndex]:
                return self.MinOrder(rotateArray, left, right)
            # 正常情况：

            if rotateArray[left] <= rotateArray[minIndex]:
                left = minIndex
            elif rotateArray[right] >= rotateArray[minIndex]:
                right = minIndex
        return rotateArray[minIndex]

    def MinOrder(self, rotateArray, left, right):
        key = rotateArray[left]
        for i in range(left, right + 1):
            if rotateArray[i] < key:
                key = rotateArray[i]
        return key





'''
#快速排序：
设要排序的数组是A[0]……A[N-1]，首先任意选取一个数据（通常选用数组的第一个数）作为关键数据，
然后将所有比它小的数都放到它前面，所有比它大的数都放到它后面，这个过程称为一趟快速排序。
值得注意的是，快速排序不是一种稳定的排序算法，也就是说，多个相同的值的相对位置也许会在算法结束时产生变动。
一趟快速排序的算法是：
1）设置两个变量i、j，排序开始的时候：i=0，j=N-1；
2）以第一个数组元素作为关键数据，赋值给key，即key=A[0]；
3）从j开始向前搜索，即由后开始向前搜索(j--)，找到第一个小于key的值A[j]，将A[j]和A[i]互换；
4）从i开始向后搜索，即由前开始向后搜索(i++)，找到第一个大于key的A[i]，将A[i]和A[j]互换；
5）重复第3、4步，直到i=j； (3,4步中，没找到符合条件的值，
即3中A[j]不小于key,4中A[i]不大于key的时候改变j、i的值，使得j=j-1，i=i+1，直至找到为止。
找到符合条件的值，进行交换的时候i， j指针位置不变。另外，i==j这一过程一定正好是i+或j-完成的时候，此时令循环结束）。
'''
import random
class sort:
    def quickSort(self,array,start,end):
        index = random.randint(0,len(array) - 1)   #随机选择一个数据作为切分数据，比这个数据小的都分到左边，比它大都分到右边
        left = start
        right = end
        key = array[start]
        while left < right:
            while array[right] >= key:
                right -= 1
            #当跳出上一个while时，此时array[right]小于key，需要将其与key换位置
            array[left] = array[right]
            while array[left] <= key:
                left += 1
            array[right] = array[left]
        array[left] = key
        self.quickSort(array,start,left-1)
        self.quickSort(array,left+1,end)
        return array


#快速排序《剑指offer》书中的答案
def Partion(array,length,start,end):
    if not array or start < 0 or end > length:
        return
    index = random.randint(start,end)
    array[end], array[index] = array[index],array[end]
    small = start - 1
    for i in range(start,end):
        if array[i] < array[end]:
            small += 1
            if small != i:
                array[small], array[i] = array[i] ,array[small]
    small += 1
    array[small] ,array[end] = array[end] ,array[small]
    return small

def QuickSort(array, length, start, end):
    if start == end:
        return
    index = Partion(array,length,start,end)
    if (index > start):
        QuickSort(array,length,start,index-1)
    if index < end:
        QuickSort(array,length,index+1,end)
    return array



